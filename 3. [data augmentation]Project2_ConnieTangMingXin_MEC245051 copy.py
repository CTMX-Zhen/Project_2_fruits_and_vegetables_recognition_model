...
# == HELPER FUNCTIONS ==
# 1. Setup logger
def setup_logger(log_file="data_augmentation_log.txt"):
    """
...
# == CUSTOM LOSS FOR LABEL SMOOTHING for build_model () ==
def sparse_categorical_crossentropy_with_label_smoothing(n_classes, smoothing=0.1):
    """
    Custom loss function for label smoothing.

    Applies label smoothing to the sparse categorical crossentropy loss function.
    ...
    Returns:
        callable: A custom loss function that applies label smoothing to the sparse categorical crossentropy loss.
    """
...
def build_model(input_shape, num_classes, base_trainable=False, freeze_until=None):
    ...
    x = tensorflow_layers.Rescaling(1./255)(inputs)
    ...
    model = tensorflow_models.Model(inputs=inputs, outputs=outputs)

    # Compile with custom loss
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=1e-5),
        loss=sparse_categorical_crossentropy_with_label_smoothing(num_classes, smoothing=0.1),
        metrics=['accuracy']
    )
...
def plot_history(history):
    ...
    # Save and show plot
    plt.savefig(os.path.join(save_dir, "data_augmentation_result.png"))
    plt.show()

# == START ==
if __name__ == "__main__":
    # Set up logging
    save_dir = "EfficientNetB0-based_image_classifier\\3. data_aug_mode"
    os.makedirs(save_dir, exist_ok=True)
    logger = setup_logger(log_file=os.path.join(save_dir, "data_augmentation_log.txt"))

    # === CONFIG ===
    data_dir = "dataset"
    img_size = (224, 224)
    batch_size = 32
    fine_tune_epochs = 5  # typically 5–10, already trained 10 epochs in stage 1 and 5 epochs in stage 2
    ...
    # Load entire model if it exists
    if os.path.exists("EfficientNetB0-based_image_classifier\\2. fine_tuned_model\\fine_tuned_model.keras"):
        logging.info("Loading full fine-tuned model...")
        model = tf.keras.models.load_model("EfficientNetB0-based_image_classifier\\2. fine_tuned_model\\fine_tuned_model.keras", compile=False)
    ...
    logging.info("Starting data augmentation training...")
    ...
    logging.info(f"Test Accuracy after data augmentation training: {test_acc:.4f}")
