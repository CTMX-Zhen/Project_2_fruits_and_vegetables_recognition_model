...
if __name__ == "__main__":
    # Set up logging
    save_dir = "EfficientNetB0-based_image_classifier\\1. base_model"
    os.makedirs(save_dir, exist_ok=True)
    logger = setup_logger(log_file=os.path.join(save_dir, "train_log.txt"))
    ...
    model = build_model(
        ...
        base_trainable=False  # Only train new head
    )
    ...
    logging.info(f"Test Accuracy (feature extraction): {test_acc:.4f}")

    # Save model
    logging.info("Training completed. Saving model...")
    model.save(os.path.join(save_dir, "base_model.keras"))
